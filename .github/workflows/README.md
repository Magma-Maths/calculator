...
why its concurrency group queues runs on `main` instead of cancelling them. A
commit on `main` that produced no image (skipped as docs-only, or cancelled by
the next push) cannot be promoted, and every commit on `main` is meant to be a
release candidate. The buildx cache keeps a no-op rebuild cheap.
